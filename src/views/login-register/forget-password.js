...
import {AppState, Image, Keyboard, ScrollView, Text, TextInput, TouchableOpacity, View, SafeAreaView, DeviceEventEmitter} from 'react-native';
...
const ForgotPassword = (props: any) => {
  ...
  const onPressGetCode = async () => {
    Keyboard.dismiss();
    await BaseService.loginpwdCode({
      auth_type: 1,
  ...
  const onComfirClick = async () => {
    ...
    await BaseService.loginpwdReset({
      auth_type: 1,
      auth_data: `+${areaCode}-${phone}`,
      password: password,
      rand_code: noteCode,
